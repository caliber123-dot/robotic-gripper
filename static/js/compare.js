...
function showhidewindow(pno) {
  ...
  document.getElementById("graphTableBody").innerHTML = `
  ...
  // clear canvas
  let canvas = document.getElementById("forceChart");
...
async function autoLoadSavedSpring() {
  ...
  if (result.status === "found") {
    ...
    // MODE 1
    if (d.k_common != null) {
      let el = document.getElementById("k_common");
    ...
    // MODE 2
    if (d.k_finger != null) {
      let el = document.getElementById("k_finger");
...
function toggleTerms(count) {
  ...
  let term2 = document.getElementById("cfunc3").closest(".eq-block");

  let thirdGroup = document.getElementById("thirdTermGroup");
...
async function calculate() {
  ...
  let data = {
    ...
    material: "rubber", // fixed material for graph page
    time: document.getElementById("time").value, // fixed time for graph page
    // func: document.getElementById("func").value,
    func: document.getElementById("func").value,
    ...
    fingers: [
      {
        k1: +document.getElementById("f1k1")?.value || 0,
        k2: +document.getElementById("f1k2")?.value || 0,
      },
      {
        k1: +document.getElementById("f2k1")?.value || 0,
        k2: +document.getElementById("f2k2")?.value || 0,
      },
      {
        k1: +document.getElementById("f3k1")?.value || 0,
        k2: +document.getElementById("f3k2")?.value || 0,
      },
      {
        k1: +document.getElementById("f4k1")?.value || 0,
        k2: +document.getElementById("f4k2")?.value || 0,
      },
    ],
  };
...
function createBarChart(timeData, forceData) {
  ...
  setTimeout(() => {
    ...
    document.getElementById("chartImage").value = chartBase64;
  }, 1000);
}
  ...
  let payload = {
    ...
    mode_name:
      document.querySelector('input[name="kmode"]:checked').value == "1"
        ? "All equal"
        : document.querySelector('input[name="kmode"]:checked').value == "2"
...
async function loadSpringConstants(gripper) {
  ...
  //   alert("Loading spring constants...", gripper);
  //   console.log("Fetching spring constants for gripper:", gripper);
  const data = await response.json();
  //   console.log("Received spring constants:", data);
...
async function loadSpringAutocomplete(springKey, datalistId) {
  //   console.log("Loading spring autocomplete for", springKey);
  let gripper = getCurrentGripper();
  let shape = document.getElementById("shape").value;
  //   let material = document.getElementById("material").value;
  let material = "rubber"; // hardcoded for now to load spring constants without selecting material and shape
  // let time = document.getElementById("time").value;
  let func = document.getElementById("func").value;
  ...
  let result = await response.json();
  //   console.log("Received spring constants:", result);
  //   console.log("Count =", result.spring_values.length);
...
// call ALL spring autocomplete functions to load options for all fields
